 ...
 *  CxdMatrix
 *
 *  Created by Chris Davey on 16/04/10.
 *  Copyright 2010 none. All rights reserved.
 *
 */

#include "TestBackprop.h"
...
CPPUNIT_TEST_SUITE_REGISTRATION (TestBackprop);



void TestBackprop::setUp (void)
{
	
}

void TestBackprop::tearDown (void)
{
	
}


void TestBackprop::test (void)
{
	...
	X = repmat(X, 8, 1); // 8 rows
	
	/*
	 State: ORGANISE_TARGET_DATA
	 */
	// a single column target vector.
	// 8 rows therefore 8 targets.
	mat targets = "1; 0; 1; 1; 0; 0; 0; 1;";
	...
	CPPUNIT_ASSERT_MESSAGE("network with 1 output neurons, Output colums does not equal input columns", 
	...
	/*
	% left output
    [ LWo, LWg, dLW ] = backprop( LC, LWo, dLW, Wa, T, Lo, Ldo, LR, 0, 0 );
    [ LWo, LWg, dLW ] = backprop( LC, LWo, dLW, Wa, Lh, Lo, Ldo, LR, 0, 0 );
    % left hidden layer
    [ LWh, LWhg, dLWh ] = backprop(LWo', LWh, dLWh, Wa, T, Lh, Ldh, LR, LWg, 1);
	*/
	/*
	Mat<Number> outputs,
